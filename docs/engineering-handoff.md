...
4. **Drift check.** The trust engine. Two stages — cheap screen (effect-list vs. declaration and footprint anomaly, both per-member, plus a bundle-wide symbol-coherence check) on every member, expensive confirm on the flagged tail only. Detailed in §6.
...
7. **Merge queue.** Lands accepted bundles as one transaction, with per-PR revert machinery behind it (INV-4, INV-5). State updates primarily via the `pull_request` webhook, backstopped by a periodic `reconcileWithGitHub()` pass that checks every active entry directly against GitHub — so a bundle doesn't stay stuck showing "queued" if a webhook delivery is missed or the webhook is unconfigured.
...
Three cheap signals — the first two run per-member; the third needs every member's touches at once, since the inconsistency it looks for only exists across PRs, not within any single one:
...
- **Symbol-coherence check.** Bundle-wide, not per-member: groups every symbol touch across all bundle members by name (each name's touches form a "face" — a k-way interaction, not just a pair), and flags a face where one member removes or renames the symbol while another member still references it. Catches the case the other two signals miss — one PR adds a name, another removes/renames it, a third still reads the old name — where no *pair* looks wrong in isolation, only the merged bundle does, and which matters precisely because a bundle lands as one merge transaction (INV-4) with no per-PR review to catch it. Deliberately does not try to reconcile a flagged removal against a co-occurring add elsewhere in the bundle (v1 has no rename-pairing or inter-PR ordering, so an "add" of the same name is indistinguishable between "this PR introduced the name" and "this PR restores what a sibling just removed") — over-flagging here matches the same high-recall posture as the other two signals.
...
- **Symbol-coherence "faces" check, a k-way set-membership scan** — `src/engine/drift/symbolCoherence/check.ts` (`findSymbolInconsistencies`). Groups every bundle member's symbol touches by name into a "face" (all touches to that name across the whole bundle, not a pairwise comparison), keeps faces touched by 2+ distinct PRs, and flags any face where a remove/rename entry co-occurs with a reference entry from a different PR. Bundle-wide by construction — it needs every member's touches in view at once to see an interaction that doesn't exist in any single PR's diff, unlike the per-member effect-list and footprint-anomaly signals above.
