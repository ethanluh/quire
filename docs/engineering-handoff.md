...
An optional, additive layer on top of everything above: an LLM-as-judge that scores each
*drift-cleared* bundle against a written constitution and, in its most autonomous mode, can
act on that score — merge, self-heal a conflict (via the existing resolver), revert on a
failed post-merge check. Full design record: `docs/judge-integration-map.md` (the integration
map written before any code existed) and `docs/judge-constitution.md` (the rubric, risk
taxonomy, and thresholds it's scored against — both human- and machine-readable).

**The one hard rule the judge exists inside of:** it never runs on a bundle the drift check
(§6) or spec-conformance check flagged. This is not a tuning knob — it's checked at both the
call site (`ingestIntoQueue.ts`) and again inside the judge's own entry point
(`orchestrate.ts`'s `runJudgeForBundle`, defense in depth). "Dishonest direction ⇒ no
autonomous action, period" is the same bargain INV-1/INV-3 already strike for the swarm,
applied one layer up: the judge is a second declaration, not a verdict on itself, and it must
never be the thing that clears a bundle a cheaper, blind check already flagged.

**Invariants preserved, not just respected:**
- **INV-1.** The judge's own `JudgeVerdict` is a declaration like `declaredDirection` — never
  trusted directly. `gate.ts` (confidence/blast-radius/reversibility/risk-taxonomy) is what
  actually decides whether it may drive an action; audit sampling (below) is the same
  discipline applied to the judge itself, one level up.
- **INV-3.** Reinforced structurally: a flagged bundle never reaches the judge, so the judge
  cannot be the thing that clears one.
- **INV-4 / INV-5.** The action pipeline (`actionPipeline.ts`) drives `MergeQueue` exclusively
  through its existing public, lock-protected methods (`enqueue`/`dequeueNext`/`revertPr`) —
  no new mutation path. Auto-accept enqueues exactly like a human accept (reversible until
  landed); revert operates per member PR, never at the bundle level.
- **INV-6.** Every verdict and every autonomous action is persisted (`judge-verdicts.json`,
  `judge-actions.json`, and append-only `instrumentation/judge-decisions.ndjson` per team) and,
  in `auto` mode, produces a Slack notification on every terminal outcome — a judge that ran
  and decided nothing is worse than one that discloses it decided nothing.

**VERIFY is deliberately read-only.** Every other integration in this codebase only ever calls
a GitHub or LLM API; the judge's post-merge verification step keeps that property rather than
having Quire's own process shell out to run a build/test command. It matches an incoming
`check_suite` webhook by `(repoOwner, repoName, merge-commit-SHA)`, plus an optional read-only
health-check GET. Ambiguous or missing signals (a webhook that never arrives, a health check
that times out) escalate as *inconclusive* — never treated as success, never triggering a
revert on a mere network blip. `QUIRE_JUDGE_AUTOFIX_CI` (a single-shot autonomous code-fix
attempt on a CI failure) was deliberately **not** built — see `.env.example` — because it would
be a materially different, larger-blast-radius capability (Quire generating and committing its
own code) than anything else here.

**Rollout path — shadow → assist → auto (`QUIRE_JUDGE_MODE`):**
1. **`off`.** True kill switch. The judge never runs, never logs. (Not one of the mission's
   original three modes — added as a superset value; see `docs/judge-integration-map.md` §7.)
2. **`shadow` (default).** Predicts, logs (`judge-verdicts.json` + the NDJSON audit trail),
   and — if `QUIRE_SLACK_WEBHOOK_URL` is set — posts a prediction of what `auto` mode *would*
   have decided. Never acts. This is the calibration phase: run it against real traffic long
   enough to trust the constitution's thresholds before ever setting `assist` or `auto`.
3. **`assist`.** Surfaces the same verdict as a `judgeRecommendation` on the `ReviewCard` (data
   layer only, as of this writing — no frontend rendering has been built for it yet, disclosed
   here per the same INV-6 standard the product holds itself to). The human's gesture is still
   the only thing that acts.
4. **`auto`.** Runs the full gated pipeline (EVALUATE → GATE → MERGE → VERIFY →
   HEAL-OR-REVERT → NOTIFY). Even here, `QUIRE_JUDGE_AUDIT_SAMPLE_RATE` (default 10%) routes a
   sample of gate-allowed verdicts to a human instead of auto-acting, so judge-vs-human
   agreement (`GET /judge/agreement`, computed live from the two logs) is measured on live
   traffic, not just retrospectively.

Do not skip a step in that order on a repo whose bundles you'd be upset to see land
unsupervised — the same phase-gate discipline §8 applies to the drift detector applies here:
each mode's exit gate is trust earned from the previous mode's logs, not a config toggle
flipped on faith.
