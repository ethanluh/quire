...
//
// findTenant resolves the team that owns this bundleId — a single callback endpoint is
// shared by every team's dispatched Action runs, so there is no longer one fixed queue to
// look the bundle up in (see TenantRegistry.findByBundleId).
export function actionCallbackRouter(
	findTenant: (bundleId: string) => Promise<{ queue: MergeQueue; conflictLogPath: string } | undefined>,
): Router {
	const router = Router();
	...
	router.post("/:bundleId/resolution", async (req, res, next) => {
		try {
			...
			const tenant = await findTenant(bundleId);
			if (tenant === undefined) {
				res.status(404).json({ error: "No pending resolution for this bundle" });
				return;
			}
			const { queue, conflictLogPath } = tenant;
