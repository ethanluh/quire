...
import type { GateDecisionLog, DriftScreenLog, JudgeVerdictLog } from "../../src/engine/types/instrumentation.js";
...
describe("createNdjsonInstrumentationSink", () => {
	...
	it("appends gate decisions, drift-screen results, and judge verdicts as separate NDJSON logs", async () => {
		...
		const judgeDecisionLogPath = join(dir, "judge-decisions.ndjson");
		const sink = createNdjsonInstrumentationSink({ gateLogPath, driftScreenLogPath, judgeDecisionLogPath });
		...
		const judgeEntry: JudgeVerdictLog = {
			bundleId: "bundle-1",
			mode: "shadow",
			status: "ok",
			gesture: "accept",
		...
		await sink.logJudgeVerdict?.(judgeEntry);
		...
		expect(JSON.parse((await readFile(judgeDecisionLogPath, "utf8")).trim())).toEqual(judgeEntry);
	});
});
